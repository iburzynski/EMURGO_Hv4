...
-- If we `fmap` the function `(a -> f b)` to `f a`, we will get a value of type `f (f b)` instead of `f b`.
...
-- Implement the Monad instance for the `Maybe` context:
