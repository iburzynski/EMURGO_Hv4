...
newtype Reader g a = Reader { runReader :: g -> a }
...
instance Functor (Reader g) where
  fmap :: (a -> b) -> Reader g a -> Reader g b
  fmap ab (Reader ga) = Reader (\g -> ab $ ga g)
  --                    same as: Reader (ab . ga)
...
instance Applicative (Reader g) where
  ...
  Reader gab <*> Reader ga = Reader (\g -> gab g (ga g))

instance Monad (Reader g) where
  (>>=) :: Reader g a -> (a -> Reader g b) -> Reader g b
  Reader ga >>= aRgb = Reader (\g -> runReader (aRgb (ga g)) g)
...
local :: (g -> l) -> Reader l a -> Reader g a
...
local gl (Reader la) = Reader (\g -> la (gl g))
--                       same as: Reader (la . gl)