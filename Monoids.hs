...
instance Semigroup Color where
  ...
  (<>) a b
    ...
    | a == Clear = b
    | b == Clear = a
    | all (\x -> x `elem` [Red, Blue, Purple]) [a, b] = Purple
    --    :: Color -> Bool
    -- all (`elem` [Red, Blue, Purple]) [Purple, Green]
    --                                  [Purple `elem` [...], Green `elem` [...]]
...
-- `mconcat` is a monoidal fold operation:
  ...
  --  we gain this method automatically by defining `mempty`
...
--   We'll use our more verbose map/mconcat version in these examples for clarity.
...
data Maybe' a = Nothing' | Just' a
  deriving Show
-- *Note: we are reimplementing the built-in `Maybe` type to illustrate its monoidal properties.
...
instance Semigroup a => Monoid (Maybe' a) where
  ...
  mconcat = foldr (<>) mempty -- implemented manually here for illustration only (not required)

-- What happens if we embed our Min/Max tuple inside a `Maybe` monoid?
...
-- folding over an empty list now returns a sensible value.
